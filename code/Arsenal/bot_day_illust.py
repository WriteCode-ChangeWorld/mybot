...
class Day_Illust:
	...
	def create_section(self,fusion_data:dict,diy_data:dict=None)->dict:
		...
		try:
			...
			log_str("已存在{}节点".format(section_name))
			return {}
	...
	def update_section(self,writedata:dict)->dict:
		...
		self.read_cfg()
		log_str("写入数据 | result: {}".format(writedata))
	...
	def judge_remind_main(self,fusion_data:dict)->dict:
		...
		log_str("judge_remind_main | section_name:{},sections_data:{}".format(section_name,len(sections_data)))
		
		# 目标节点不在配置文件节点列表中
		if section_name not in sections_data:
			log_str("judge_remind_main | Empty Data:{}".format(section_name))
		...
		# 非今日日期,则进行签到
		if u_data["date"] != date:
			log_str("judge_remind_main | Update Date")
			u_data['is_remind'] = "False"
			return u_data
		# 未触发过每日抽图
		elif u_data['is_remind'] == "False":
			log_str("judge_remind_main | User is_remind False")
			return u_data
		else:
			log_str("judge_remind_main | User is_remind True")
			return u_data
	...
	def day_illust_process(self,eval_cqp_data):
		...
		if eval_cqp_data.get('message_type','') == 'group' and \
			...
			log_str("本地数据 | result: {}".format(result))
			# log_str("测试原始数据: {}".format(backup_result))
			...
			if result.get("is_remind","") == "False":
				...
				log_str("基础概率: {} 发言增幅概率: {} 天选之人概率: {} 保留概率: {}".format(\
				...
				if not self.check_express_data("group_id",group_id,"is_koi",True):
					...
					log_str("next_koi_random 天选之人抽取概率: {}".format(next_koi_random))
				...
				if group_id in self.extra_group_list:
					...
					log_str("最低阈值: {}".format(limit_prob))
					if limit_prob < 0.0:
						limit_prob = 0.0
					illust_level = self.get_rare_value(prob,limit_prob=limit_prob)
					log_str("in limit group |illust_level: {}".format(illust_level))
				else:
					# 魔方
					...
					log_str("not in limit group | illust_level: {}".format(illust_level))
				...
				# log_str(prob_interval_static,prob_interval_text)
				...
				if illust_flag == True and not new_path:
					self.update_section(backup_result)
					log_str("NotGBImgPath backup_result: {}↓".format(backup_result))
					return 
				# 未抽到卡的模板消息
				# is_remind此时已为True,无需改动
					...
					log_str("Miss Img: {}".format(result))
				# 抽到卡的模板消息
				else:
					# retention_prob清零
			...
			# 发言数统计
				...
				log_str("更新前总用户长度:{}".format(len(self.get_sections())))
				self.update_section(result)
				log_str("更新后总用户长度:{}".format(len(self.get_sections())))
				return data
	...
	def get_rare_value(self,rare_random,limit_prob=0.0,rare_set=None):
		...
		log_str("get_rare_value | 稀有度分布 pool_rare_set: {}".format(pool_rare_set))
		log_str("get_rare_value | 随机值 rare_random: {}".format(rare_random))

		start_rare = 0
		# 0~稀有度分布最右侧
		for k,v in pool_rare_set.items():
            # 各稀有度右侧边界值 = 上一次右侧边界值 + 本轮稀有度占比
			# log_str(start_rare, round((start_rare + v),3))
...
"""
cqp.py调用
from bot_day_illust import Bot_Day_Illust,log_str
Day_Illust_Data = Bot_Day_Illust.day_illust_process(eval_cqp_data)
log_str("Day_Illust_Data:{}".format(Day_Illust_Data))
